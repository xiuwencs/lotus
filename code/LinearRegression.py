...
import numpy as np
import NgramSegment as Ngram
import MessageSegment as MS
import MessageSegment as msg
import binascii
import Classifier_protocol as CP
...
def PrimaryEquation(ngrams: list, max_gramdimension: int) -> list:
    ...
    for i in range(len(ngrams)):  # i: 每条数据包
        ...
        for j in range(len(ngrams[i])):  # j 每条数据包的每个gram
            grams_list.append(BigEndian(ngrams[i][j]))
...
def ascii_to_integer(ascii_code):
    chars = [ascii_code[i:i + 2] for i in range(0, len(ascii_code), 2)]
...
def DimensionReduction(primary_equation: list, max_gramdimension: int, ngrams: list, infer_len: list) -> list:
    ...
    """
    right_column = []
    primary = np.array(primary_equation)
    # print("primary:", primary_equation)
    for i in range(max_gramdimension):  # 方程的第i列
        ...
        if fieldvariety != packetlenvariety:  # 与数据包长种类数量不一致的字段列过滤
            flag = False
        if flag:
            if primary_equation[0][i] != 0:
                if CP.flag == 0:
                    if i + 1 < len(primary_equation[0]):
                        hex_value = hex(primary_equation[0][i])[2:]
                        hex_value1 = hex(primary_equation[0][i + 1])[2:]
                        # print("i+1(hex):",hex_value1)
                        str_hex_value1 = str(hex_value1)
                        if '0d0a' or '0d' in str_hex_value1:
                            str_length = str(primary_equation[0][i])
                            str_hex = str(ngrams[0][i])
                            count_of_zeros = str_hex.count('0')
                            # print("count_of_zeros:",count_of_zeros)
                            if len(str_hex) - 4 - count_of_zeros == len(hex_value) and count_of_zeros != 0:
                                if '0d' not in ngrams[0][i] and '0a' not in ngrams[0][i] and '20' not in ngrams[0][
                                    i] and '0d0a' not in ngrams[0][i]:
    ...
    final_equation = np.array(final_equation)
...
packet_content, infer_len, real_len = MS.MessageSegment(file, ran, gap)
...
if CP.flag == 1:#是二进制协议，使用n-gram方法
    for member in range(1, 5):
        ...
        print("筛选后的列索引(字段偏移量)：")
        new_column = list(set(right_column))
        print(new_column)
        temp = []
        temp1 = []
        for index in new_column:
            temp.append(ngrams[0][index])
        print("筛选后的列的内容")
        print(temp)
...
else:#是文本协议，直接使用语义信息的方法
    text = []
    for sublist in packet_content:
        extracted = []
        for item in sublist:
            if '3a20' in item:
                extract_text = item.split('3a20')[1]
                exrtact_value = ascii_to_integer(extract_text)
                if exrtact_value.isdigit():
    ...
    # 记录最大维度
    max_gramdimension = MaxDimension(text)

    # 建立初始方程
    primary_equation = PrimaryEquation(text,
                                       max_gramdimension)
    # 降维
    final_equation, right_column = DimensionReduction(primary_equation, max_gramdimension, text, infer_len)
    new_column = list(set(right_column))
