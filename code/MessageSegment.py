...
# 引用包
import random
import TextSimilarityMeasure as TextSimilarityMeasure
...
def process_packet_content(packet_content):
    new_packet_content = []
    for sublist in packet_content:
        for i in range(0, len(sublist) - 1):
            if i + 2 < len(sublist) - 1:
                if sublist[i] == '0d0a' and sublist[i + 2] == '0d0a':
                    data = sublist[i + 1]
                    # print("data:",data)
                    k = data.find('3a20')
                    # print("k:",k)
                    ...
                    if ascii_data.isdigit():
                        decimal_list = [int(ascii_data[j:j + 2]) - 30 for j in range(0, len(ascii_data), 2)]
                        try:
                            decimal_number = int(''.join(map(str, decimal_list)))
                        except ValueError:
                            decimal_number = 0
                        hex_number = hex(decimal_number)[2:]
                        hex_number = hex_number.zfill(len(ascii_data))
                        # print("hex_value:", hex_number)
                        if hex_number:
                            new_data = data.replace(ascii_data, hex_number)
                            # print("new_data:",new_data)
                    else:
                        new_data = data
                    sublist[i + 1] = new_data

        new_packet_content.extend(sublist)
        restore_packet_content = []
        start = 0
        for sublist in packet_content:
            length = len(sublist)
            restore_packet_content.append(new_packet_content[start:start + length])
            start += length
    return restore_packet_content
...
def MessageSegment(file, ran: int, gap: int) -> list:
    ...
    """
    # 读取字段划分的文件
    FieldSegment_File = '/home/lyn/txt/sip_rose.txt'
    ...
    # 计算最大频率偏移量差值
    maxfreq_offdif, recomb_offsetdiff, recomb_offsetdiffdic = PacketOffset(
        recomb_cluster)  # recomb_offsetdiff 记录每条重组包的各种偏移量插值
    ...
    gap_num, maxfreq_fieldcomb = FieldComb_gap(sortdict, maxfreq_offdif_packetindex, recomb_offsetdiffdic,
                                               maxfreq_offdif)
    print("更新后的分割字段")
    print(maxfreq_fieldcomb)
    print('字段间隔距离')
    ...
    print("推断包长准确率：")
    print(acc)
    return packet_content, infer_packetlen, real_packetlen
